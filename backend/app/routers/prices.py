...
def get_time_patterns(
    ...
    unique_days = db.query(
        func.count(func.distinct(func.date(FuelPrice.timestamp)))
    ).filter(
        FuelPrice.timestamp >= since,
        fuel_column.isnot(None)
    ).scalar() or 0
    ...
    day_results = db.query(
        ...
        FuelPrice.timestamp >= since,
        fuel_column.isnot(None)
    ).group_by('day_of_week').order_by('avg_price').all()
    
    # Cheapest by hour of day
    hour_results = db.query(
        func.extract('hour', FuelPrice.timestamp).label('hour_of_day'),
        func.avg(fuel_column).label('avg_price')
    ).filter(
        FuelPrice.timestamp >= since,
        fuel_column.isnot(None)
    ).group_by('hour_of_day').order_by('avg_price').all()
...
def get_go_now_indicator(
    ...
    current_avg = db.query(
        func.avg(fuel_column)
    ).filter(
        FuelPrice.timestamp >= one_hour_ago,
        fuel_column.isnot(None)
    ).scalar()
    ...
    day_avg = db.query(
        ...
        FuelPrice.timestamp >= one_day_ago,
        fuel_column.isnot(None)
    ).scalar()
...
def get_top_stations(
    ...
    station_stats = db.query(
        ...
        Station.brand,
        Station.city
    ).order_by(func.avg(fuel_column)).limit(limit).all()
    ...
    cheapest_counts = {}
    overall_min = db.query(func.min(fuel_column)).filter(
        FuelPrice.timestamp >= since,
        fuel_column.isnot(None)
    ).scalar() or 0
    ...
    for stat in station_stats:
        ...
        if overall_min > 0:
            ...
            if price_diff < 0.01:
                cheapest_counts[stat.station_id] = int(total_samples * 0.3)  # Rough estimate
    ...
    for stat in station_stats:
        ...
        cheapest_percentage = (cheapest_count / total_samples) * 100 if total_samples > 0 else 0
...
def get_lowest_price_ever(
    ...
    result = db.query(
        ...
        Station, FuelPrice.station_id == Station.id
    ).filter(
        fuel_column.isnot(None)
    ).order_by(
        fuel_column.asc()
