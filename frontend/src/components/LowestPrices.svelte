<script>
  ...
  async function fetchLowest() {
    try {
      const response = await fetch(`http://localhost:8000/api/prices/lowest?fuel_type=${fuelType}&hours=${hours}&limit=5`)
      lowestPrices = await response.json()
      
      // Fetch full station details for the cheapest one
      if (lowestPrices.length > 0) {
        const stationResponse = await fetch(`http://localhost:8000/api/stations/${lowestPrices[0].station_id}`)
